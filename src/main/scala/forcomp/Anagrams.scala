...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def isGoodComb(charactors: List[Char]): Boolean = {
       val n =
          for {
            key <- charactors

            if (occurrences.map { case (a, b) => a }.contains(key))

          } yield key

          (n.length == charactors.length)
    }

    val occurrenceSubsets = for {
      (dictKeys, _) <- dictionaryByOccurrences
      //elementExistOccur <- ((dictKeys.map { case (a, b) => a }).filter( occurrences.map { case (a, b) => a }.contains(_) == false ).length == 0)
      //elementDoesExist <-  (dictKeys.map { case (a, b) => a } exists (char => occurrences.map { case (a, b) => a }.contains(char) == false ))

        if(isGoodComb(dictKeys.map { case (a, b) => a }))

      } yield dictKeys



    List() :: (occurrenceSubsets toList)
  }
